...
{
    // init B signal callback function (function pointer)
    sCallback = fptr;
}
 ...
 * can be handled
 */
bool setAspect(uint8_t index, uint8_t aspect)
{
    if (aspect == 0)
    {
...
bool isAspectValid(uint8_t oldAspect, uint8_t newAspect)
{
    ...
    if ((oldAspect == 0x00) && (newAspect == 0x01)) { return true; }
    if ((oldAspect == 0x00) && (newAspect == 0x02)) { return true; }
    if ((oldAspect == 0x00) && (newAspect == 0x03)) { return true; }
    if ((oldAspect == 0x02) && (newAspect == 0x03)) { return true; }
