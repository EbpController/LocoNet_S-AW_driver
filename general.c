...
void updateLeds(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        ledOutput = 0x00;
        // KFS
        if (s[i].KFS == true) { ledOutput |= 0x01; }
        // KOS
        if (s[i].KOS == true) { ledOutput |= 0x02; }
        // CAWL
        if (aw[i].CAWL == true) { ledOutput |= 0x20; }
        // KAWL
        if (aw[i].KAWL == true) { ledOutput |= 0x10; }
        // CAWR
        if (aw[i].CAWR == true) { ledOutput |= 0x40; }
        // KAWR
        if (aw[i].KAWR == true) { ledOutput |= 0x80; }
        ...
        // W
        if (s[i].intensity.W >= pwmCounter) { ledOutput |= 0x80; }
        // YV
        if (s[i].intensity.YV >= pwmCounter) { ledOutput |= 0x40; }
        // R
        if (s[i].intensity.R >= pwmCounter) { ledOutput |= 0x20; }
        // G
        if (s[i].intensity.G >= pwmCounter) { ledOutput |= 0x10; }
        // YH
        if (s[i].intensity.YH >= pwmCounter) { ledOutput |= 0x08; }
        // BA1
        if (s[i].intensity.BA1 >= pwmCounter) { ledOutput |= 0x04; }
        // BA2
        if (s[i].intensity.BA2 >= pwmCounter) { ledOutput |= 0x02; }
        // send signal state to led outputs
        MAX7219_send(i + 1, ledOutput);
...
{
    // this could be used to handle the (changed) state of KFS or KOS
}
